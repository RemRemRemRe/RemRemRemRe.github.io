...
1.  对`QueryTokenStream`的第0和1号元素写入"版本号"和"是否含有逻辑表达式"的信息
1.  对`逻辑表达式`进行解析,在`QueryTokenStream`中写入表达式类型的枚举,对应`EGameplayTagQueryExprType`,对于"非嵌套表达式"类型,写入它用到的tag数量,将每个tag去重添加到`TagDictionary`,并写入下标;对于"嵌套表达式"类型,递归解析
